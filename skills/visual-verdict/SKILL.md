...
- Optional `category_hint`: for example `dashboard`, `landing-page`, `sns-feed`, `docs-site`, or `mobile-app`.
</Inputs>
...
- `differences[]`: concrete visual mismatches in layout, spacing, typography, color, hierarchy, and component state.
...
- Persist the verdict in `.oh-my-openclaw/state/<scope>/ralph-progress.json` or an equivalent task artifact with both:
...
2. Use pixel-level diff tooling as a **secondary debug aid** to localize hotspots.
...
```json
{
  ...
  "category_match": true,
  "differences": [
    "Top nav spacing is tighter than the reference",
